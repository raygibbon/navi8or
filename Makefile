...
.PHONY: external-url-test viewer-workflow-test
external-url-test: | build
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/external_url_test.c src/platform/posix.c src/platform/external_url.c $(LDFLAGS) -o build/external-url-test
	./build/external-url-test

viewer-workflow-test: nav external-url-test viewer-pane-state-test
	python3 tests/viewer_workflow_test.py ./nav ./build/external-url-test
	python3 tests/viewer_panes_test.py ./nav

.PHONY: viewer-pane-state-test
viewer-pane-state-test: nav
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/viewer_pane_state_test.c $(filter-out $(OBJECT_DIR)/main.o,$(OBJECTS)) $(OBJECT_DIR)/toml.o $(LDFLAGS) -o build/viewer-pane-state-test $(LDLIBS)
	./build/viewer-pane-state-test

check: viewer-workflow-test
...
config-test: | build
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/config_test.c src/config.c src/theme.c src/symbols.c $(INPUT_SOURCES) src/platform/posix.c src/platform/external_url.c third_party/toml.c $(LDFLAGS) -o build/config-test
	./build/config-test

profile-test: | build
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/profile_test.c src/profile.c src/config.c src/theme.c src/symbols.c $(INPUT_SOURCES) src/platform/posix.c src/platform/external_url.c third_party/toml.c $(LDFLAGS) -o build/profile-test
	./build/profile-test
...
.PHONY: profile-test
theme-test: | build
	$(CC) $(CPPFLAGS) $(CFLAGS) tests/theme_test.c $(INPUT_SOURCES) src/ui/layout.c src/config.c src/theme.c src/symbols.c src/platform/posix.c src/platform/external_url.c third_party/toml.c $(LDFLAGS) -o build/theme-test
	./build/theme-test
